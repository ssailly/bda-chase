...
public class Parser {
	...
	private static EGD parseEGD(String s) {
		...
		for(String phiAtom : phiList) {
			if(phiAtom.charAt(phiAtom.length() - 1) == ')') phi.add(createRelationalAtom(phiAtom));
	...
	private static TGD parseTGD(String s) {
		...
		for(String phiAtom : phiList) phi.add(createRelationalAtom(phiAtom));
		for(String psiAtom : psiList) psi.add(createRelationalAtom(psiAtom));
	...
	private static Dependency.RelationalAtom createRelationalAtom(String atom) {
	...
	public static List<Dependency> parse(String filepath) throws FileNotFoundException {
		...
		while (sc.hasNextLine()) {
			String line = sc.nextLine(), toParse = line.substring(5);
